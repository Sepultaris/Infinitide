...
namespace ACE.Server.WorldObjects
{
    partial class Player
    {
            ...
            if (CurrentLandblock != null)
            {
                if (this is Player player && player.CurrentLandblock.HasDungeon)
                {
                    if (xpType == XpType.Kill)
                    {
                        ...
                        var lb = LandblockManager.GetLandblockUnsafe(CurrentLandblock.Id, Location.Instance); //Get current instanced landblock
                        var players = lb.GetAllCreatures().Where(x => x is Player);
                        var instanceLevel = Location.CalculateInstanceLevel(CurrentLandblock.Id);

                        m_amount = Location.CalculateScaledXp(m_amount, instanceLevel);
                    }
                }
            }
        ...
        public virtual void GrantXP(long amount, XpType xpType, ShareType shareType = ShareType.All)
        {
            if (IsOlthoiPlayer)
            {
                if (HasVitae)
        ...
        /// </summary>
        protected virtual void UpdateXpAndLevel(long amount, XpType xpType)
        {
        ...
        protected void UpdateXpVitae(long amount)
        {
        ...
        /// </summary>
        public virtual long? GetRemainingXP(uint level)
        {
            var maxLevel = GetMaxLevel();
        ...
        /// </summary>
        public virtual ulong GetRemainingXP()
        {
            var maxLevel = GetMaxLevel();
            if (Level >= maxLevel)
                return 0;
        ...
        /// </summary>
        public virtual long GetXPBetweenLevels(int levelA, int levelB)
        {
            ...
            var levelA_totalXP = (long) DatManager.PortalDat.XpTable.CharacterLevelXPList[levelA];
            var levelB_totalXP = (long) DatManager.PortalDat.XpTable.CharacterLevelXPList[levelB];
        ...
        public virtual void CheckForLevelup()
        {
            ...
            while ((TotalExperience ?? 0) >= (long) xpTable.CharacterLevelXPList[(Level ?? 0) + 1])
            {
                Level++;
                ...
                if (Level == maxLevel)
                {
                    ...
                    TotalExperience = TotalExperience - 4000000000;
                    break;
                }
            }
